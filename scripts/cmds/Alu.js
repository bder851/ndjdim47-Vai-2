...
module.exports.config = {
  ...
  premium: true,
  guide: "{pn} [prompt]\nExample: {pn} cute anime dog",
  countDown: 15,
};

module.exports.onStart = async ({ event, args, api }) => {
  const animemixApi = "https://renzweb.onrender.com/api/animemix"; // Verify if this is correct
  ...
  try {
    ...
    const apiUrl = `${animemixApi}?prompt=${encodeURIComponent(prompt)}&apiKey=${encodeURIComponent(apiKey)}`;
    ...
    if (e.response && e.response.status === 404) {
      errorMessage = "API endpoint not found (404). Please check if 'animemix' is the correct endpoint.";
    } else if (e.message) {
